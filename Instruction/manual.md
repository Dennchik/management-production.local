...
3.5 — Схема получается примерно такая:
...
```
1. Список приходных ордеров — /receipts. 

2. Просмотр конкретного приходного ордера. 

3. В списке показывать: 
...
4. Переход из списка в карточку ордера. 

5. Проверить связь приход → рулон → остаток на складе. 

 После этого перейти к Расходному ордеру.
```